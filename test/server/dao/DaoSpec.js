...
var fixture = require('../../../server/dao/Dao');
...
describe('DAO', function() {
	...
	it('Connection is released back to the pool upon error', function(done) {
		...
		async.each(badQueries, function(badQ, callback) {
			fixture.findAll(badQ, [], function(err, result) {
	...
	it('Find all users returns results', function(done) {
		...
		var queryParams = [];
		fixture.findAll(queryString, queryParams, function(err, results) {
	...
	it('Supports multiple statements', function(done) {
		...
		var params = [2, 3];
		fixture.multiStatement(statements, params, function(err, results) {
	...
	describe('Unique Constraints', function() {
		...
		it('Cannot insert duplicate QCER', function(done) {
			...
			fixture.insertOrUpdateRecord(queryString, jsonData, function(err, insertedId) {
	...
	describe('Handle Result', function() {

		it('Calls back with error if error provided', function(done) {
			...
			fixture.handleResult(cb, testError);
		});

		it('Calls back with result if error is null', function(done) {
			var testError = null;
			var testResult = {foo: 'bar'};
			var cb = function(err, result) {
				expect(err).to.be.null;
				expect(result).to.equal(testResult);
				done();
			};
			fixture.handleResult(cb, testError, testResult);
		});
