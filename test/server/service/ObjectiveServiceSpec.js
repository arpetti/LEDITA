...
describe('Objective Service', function() {
	...
	describe('Remove Aim', function() {

		it('Removes an aim', function(done) {
			var ldId = 200;
	        ...
	        var deleteResult = 0;
	        var deleteDaoStub = sandbox.stub(LdEditDao, "deleteAim", function(ldId, objectiveId, callback) {
			...
			ObjectiveService.removeAim(ldId, objectiveName, serviceCB);
		});
		...
		it('Calls back with error and message if Reference Dao calls back with error', function(done) {
	        ...
	        var deleteDaoStub = sandbox.stub(LdEditDao, "deleteAim");
	        ...
	        var serviceCB = function(err, message) {
	        	...
	        	expect(message).to.equal(messages.OBJECTIVE_REMOVE_FAIL);
	        	assert.isTrue(refDaoStub.withArgs(listMatcher).calledOnce);
