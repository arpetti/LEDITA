...
angular.module('activityDurationDisplay', []).filter('durationDisplay', function() {
    	return function(input) {
    			...
    			var hourDisplay = (input.dur_hh && input.dur_hh != 0) ? input.dur_hh + 'hh' : null;
    			var dayDisplay = (input.dur_dd && input.dur_dd != 0) ? input.dur_dd + 'day' : null;
    			var monthDisplay = (input.dur_mon && input.dur_mon != 0) ? input.dur_mon + 'mon' : null;

    			function isNotNull(element) {
  					return element != null;
				}
    			return [minuteDisplay, hourDisplay, dayDisplay, monthDisplay].filter(isNotNull).join(" ");
        };
});